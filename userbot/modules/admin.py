...
async def rm_deletedacc(show):
    ...
    # Well
    if not admin and not creator:
        return await show.edit("`Senpai Bukan Admin Disini!`")

    await show.edit("`Menghapus Akun Terhapus...\nMohon Menunggu Iblis Sedang Dalam Proses`")
    del_u = 0
    del_a = 0
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                return await show.edit("`Senpai Tidak Memiliki Izin Banned Dalam Grup Ini`")
...
async def get_usersdel(show):
    ...
    try:
        ...
        await show.edit(
            "Senpai, Grup Ini Terlalu Besar, Mengunggah Daftar Akun Terhapus Sebagai File."
        )
        file = open("daftarpengguna.txt", "w+")
        file.write(mentions)
        file.close()
