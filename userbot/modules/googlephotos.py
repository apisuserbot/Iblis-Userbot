...
async def create_token_file(token_file, event):
    ...
    async with event.client.conversation(event.chat_id, timeout=600) as conv:
        await conv.send_message(
            "Pergi Ke "
            "Linknya Dan Ikuti "
            f"Browser Anda Senpai: {authorize_url} Dan "
            "Balas Kode"
        )
...
async def upload_google_photos(event):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        real_upload_url = step_one_resp_headers.get("Iblis-Goog-Upload-URL")
        logger.info(real_upload_url)
        upload_granularity = int(
            step_one_resp_headers.get("Iblis-Goog-Upload-Chunk-Granularity")
        )
