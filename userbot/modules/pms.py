    ...
    if event.is_private:
        if not pm_permit_sql.is_approved(chats.id):
            pm_permit_sql.approve(
                chats.id, "`Sensei Rimuru Telah Mengirimi Anda Pesan 😯`")
            await borg.send_message(
                chats, "**Menerima Pesan!, Pengguna Terdeteksi Adalah Sensei Rimuru**"
            )
